...
export default function Doctors() {
  ...
  const loadDoctors = async () => {
    ...
    try {
      ...
      console.error(err);
      setError('Unable to fetch doctors list from Flask backend. Please ensure backend is running.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50/70 py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-sky-700 bg-sky-50 px-3.5 py-1 rounded-full border border-sky-200 mb-3">
            ...
            <span>SIMSRH Specialists Directory • Tumakuru</span>
          </div>
          <h1 className="text-2xl sm:text-4xl font-black text-slate-900 tracking-tight mb-2">
            Find & Book Top Doctors
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 max-w-3xl leading-relaxed">
            Consult verified clinical specialists across 10 hospital departments, review patient feedback ratings, view live OPD wait times, and reserve your consultation token.
          </p>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs mb-8 space-y-5">
          ...
          <div className="flex flex-col lg:flex-row gap-4 justify-between items-stretch lg:items-center">
            ...
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search doctors, Cardiology, fever, knee pain, acne, migraine..."
                value={searchTerm}
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <label className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer select-none bg-emerald-50/70 border border-emerald-200/80 px-3 py-1.5 rounded-xl">
                ...
                />
                <span className="text-emerald-900">In Clinic Today</span>
              </label>
        ...
        <div className="flex items-center justify-between mb-6 text-xs text-slate-500">
          <div>
            Showing <span className="font-extrabold text-slate-900">{filteredDoctors.length}</span> doctor{filteredDoctors.length === 1 ? '' : 's'}
            {selectedDept !== 'All' && <span> in <span className="font-extrabold text-sky-700">{selectedDept}</span></span>}
            {searchTerm && <span> matching "<span className="font-extrabold text-slate-800">{searchTerm}</span>"</span>}
          </div>
          <Link
            to="/departments"
          ...
          >
            ...
            <span>Browse Clinical Wings</span>
          </Link>
        </div>

        {/* Doctors Grid */}
        {loading ? (
          <LoadingState message="Loading verified doctors and live OPD availability..." />
        ) : error ? (
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-slate-200/80 shadow-xs max-w-lg mx-auto">
            ...
            <p className="text-xs text-slate-500 mb-6">
              No specialists match your search criteria. Try clearing filters or searching for another condition.
            </p>
            <button
              onClick={() => {
                setSearchTerm('');
      ...
      {showLoginModal && (
        <LoginRequiredModal
          ...
          message="Please log in with your phone or email to join a doctor's live OPD queue."
        />
      )}
