      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <div className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200 mb-2">
              ...
              <span>{t('hospital_ops_analytics', 'Hospital Operational Analytics')}</span>
            </div>
            <h1 className="text-2xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
              {t('queue_opd_analytics', 'Real-Time Queue & OPD Analytics')}
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              {t('live_metrics_desc', 'Live operational metrics aggregated from active MongoDB queue records at Shridevi Hospital.')}
            </p>
          </div>
          ...
          >
            ...
            <span>{t('refresh_analytics', 'Refresh Analytics')}</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
                {t('active_in_queue_card', 'Active In Queue')}
              </span>
            ...
            <p className="text-xs text-slate-500 font-medium">{t('patients_waiting_desc', 'Patients waiting or in consultation')}</p>
          </div>

          <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
                {t('emergency_ratio', 'Emergency Ratio')}
              </span>
            ...
            <p className="text-xs text-rose-700 font-medium">
              {emergencyCount} {t('emergency', 'Emergency')} / {Math.max(0, totalWaiting - emergencyCount)} {t('normal', 'Normal')}
            </p>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
                {t('avg_ai_predicted_wait', 'Avg AI Predicted Wait')}
              </span>
            ...
            <p className="text-xs text-emerald-800 font-medium">{t('rf_ml_forecast', 'Random Forest ML forecast')}</p>
          </div>

          <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
                {t('consultations_completed_card', 'Consultations Completed')}
              </span>
            ...
            <p className="text-xs text-teal-800 font-medium">{t('discharged_today', 'Discharged OPD visits today')}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
          ...
          <div className="lg:col-span-7 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
              <div>
                <h3 className="text-base font-bold text-slate-900">{t('queue_dist_by_dept', 'Queue Distribution by Department')}</h3>
                <p className="text-xs text-slate-500">{t('live_patient_load', 'Live patient load across specialized hospital OPD wings')}</p>
              </div>
            ...
            <div className="space-y-4">
              {Object.entries(deptDist).map(([deptName, count]) => {
                ...
                const translatedDept = t('dept_' + deptName.toLowerCase().replace(/[^a-z]/g, '_'), deptName);
                return (
                  <div key={deptName} className="space-y-1.5">
                    <div className="flex justify-between text-xs font-semibold">
                      <span className="text-slate-800">{translatedDept}</span>
                      <span className="text-sky-700">{count} {t('patients', 'Patients')}</span>
                    </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs space-y-4">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                ...
                <span>{t('peak_traffic_patterns', 'Peak Traffic & Arrival Patterns')}</span>
              </h3>
